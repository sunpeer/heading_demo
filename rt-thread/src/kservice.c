...
static rt_device_t __ulog_device;

rt_device_t rt_ulog_get_device(void)
{
    return __ulog_device;
}
RTM_EXPORT(rt_console_get_device);
/**
...
rt_device_t rt_kprintf_set_device(const char* name)
{
    __ulog_device=rt_device_find(name);
    rt_device_open(__ulog_device,RT_DEVICE_FLAG_STREAM);
    return __ulog_device;
}
RTM_EXPORT(rt_kprintf_set_device);
...
void rt_kprintf(const char *fmt, ...)
{
    ...
    rt_hw_console_output(rt_log_buf);
/*
    rt_device_write(kprintf_device, 0, rt_log_buf, length);
*/
#endif
    va_end(args);
}
